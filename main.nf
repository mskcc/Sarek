...
process MapReads {
  ...
  if (SarekUtils.hasExtension(inputFile1,"fastq.gz"))
    ...
    samblaster --addMateTags --maxSplitCount 2 --minNonOverlap 20 | sambamba view -t ${task.cpus} -S -f bam -l 0 /dev/stdin | sambamba sort --tmpdir ./ -t ${task.cpus} -m ${task.memory.toString().split(" ")[0].toInteger()}G -o ${idRun}.bam /dev/stdin
    """
